...
router.post('/verify', async (req, res) => {
	...
	if (token == undefined || token == {})
		return res.json({
			status: "error",
			message: "verify token cannot be empty"
		})
...
router.post('/login', async (req, res) => {
	...
	if (req.body == undefined || req.body == {} || req.body.user == undefined)
		return res.json({
			status: "error",
			message: " Invalid Request"
		})
	else if (req.body.user.email == '' || req.body.user.email == undefined || req.body.user.password == '' || req.body.user.password == undefined) {
		return res.json({
			status: "error",
...
router.post('/generate', verifyToken, async (req, res) => {
	...
	if (req.body == undefined || req.body == {} || req.body.url == undefined)
		return res.json({
			status: "error",
			message: " Invalid URL passed"
		})
	else if (req.body.url.orignal == '' || req.body.url.orignal == undefined) {
		return res.json({
			status: "error",
			message: " Invalid URL passed"
		})
		...
		jwt.verify(req.token, process.env.JWT, async (err, data) => {
			if (err) {
				...
				//console.log(data)
				if (req.body.url.passed == undefined) {
					return res.json({
						status: "error",
...
router.post('/getlink', async (req, res) => {
	...
	else if (req.body.url.hashLink == '' || req.body.url.hashLink == undefined) {
		return res.json({
			status: "error",
			message: " Invalid URL passed"
		})
		...
		if (err) {
			...
			if (data == undefined || data == null || data.length == 0)
				...
				if (data[0].expiresAt < Date.now()) {
					model.urlModel.deleteOne({ id: data[0]._id }, (err, response) => {
						if (err)
							return res.json({
								status: "error",
								message: err
